...
class MenuPage(tk.Frame):
    def __init__(self, master):
        ...
        start_label = tk.Label(self, text="Password Checker\n", font=(None, 24))
        start_label.grid(row=0, column=0, sticky=tk.NSEW)
        start_button = tk.Button(
            self, text="Check Password", command=lambda: master.switch_frame(MainPage)
        )
        start_button.grid(row=1, column=0, sticky=tk.NSEW)
        credits_button = tk.Button(
            self, text="Configure Parameters", command=lambda: master.switch_frame(SettingsPage)
        )
        credits_button.grid(row=2, column=0, sticky=tk.NSEW)
        quit_button = tk.Button(self, text="Quit", command=lambda: sys.exit())
...
class SettingsPage(tk.Frame):
    def __init__(self, master):
        ...
        tk.Label(frame_1, text="\nLength of Password:", font=("", 12)).pack(side=tk.LEFT, fill=tk.BOTH)
        length_scale = tk.Scale(frame_1, from_=3, to=30, orient=tk.HORIZONTAL)
        length_scale.pack()
        self.grid_rowconfigure(0, weight=1)
        self.grid_rowconfigure(1, weight=1)
        ...
        tk.Button(
            end_frame, text="Save", command=lambda: self.save_settings(master, length_scale.get())
        ).pack(side=tk.LEFT)
        tk.Button(
...
class MainPage(tk.Frame):
    def __init__(self, master):
        ...
        text1 = tk.Label(self, text="Password contains more than {} characters".format(password_length))
        ...
        button1 = tk.Button(self, text="Submit", command=lambda: self.checkPass(self.entry1.get()))
        button1.grid(column=2, row=3, sticky=tk.W)

        button2 = tk.Button(self, text="Back", command=lambda: master.switch_frame(MenuPage))
        button2.grid(column=0, row=4, pady=10)

    def checkPass(self, text):
        if len(text) > password_length:
